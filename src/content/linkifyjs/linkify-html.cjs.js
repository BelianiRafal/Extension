...
var Tokenizer = /** @class */ (function () {
  ...
  Tokenizer.prototype.appendToTagName = function (char) {
    this.current("StartTag" /* StartTag */, "EndTag" /* EndTag */).tagName +=
      char;
  };
  // Tags - attributes
...
function linkifyHtml(str, opts) {
  ...
  const linkified = [];
  const options = new linkifyjs.Options(opts, defaultRender);
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.type === StartTag) {
      ...
      const isIgnored =
        tagName === "A" || options.ignoreTags.indexOf(tagName) >= 0;
      if (!isIgnored) {
        continue;
      }
...
function skipTagTokens(tagName, tokens, i, skippedTokens) {
  ...
  while (i < tokens.length && stackCount > 0) {
    ...
    if (token.type === StartTag && token.tagName.toUpperCase() === tagName) {
      ...
      token.type === EndTag &&
      token.tagName.toUpperCase() === tagName
    ) {
      // Closing tag
      stackCount--;
    }
