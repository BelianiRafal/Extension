var linkify = (function (exports) {
   ...
   * @param {Collections<T>} [groups]
   */
  const ta = (state, input, next, flags, groups) =>
    state.ta(input, next, flags, groups);
   ...
   * @param {Collections<T>} [groups]
   */
  const tr = (state, regexp, next, flags, groups) =>
    state.tr(regexp, next, flags, groups);
   ...
   * @param {Collections<T>} [groups]
   */
  const ts = (state, input, next, flags, groups) =>
    state.ts(input, next, flags, groups);
   ...
   * @param {Flags} [flags]
   */
  const tt = (state, input, next, flags, groups) =>
    state.tt(input, next, flags, groups);
  ...
  function run$1(start, str) {
    ...
    while (charCursor < charCount) {
      ...
      while (
        ...
        (nextState = state.go(iterable[charCursor]))
      ) {
        state = nextState;
  ...
  MultiToken.prototype = {
    ...
    toFormattedHref(options) {
      return options.get(
        "formatHref",
        this.toHref(options.get("defaultProtocol")),
        this,
      );
    },
    /**
  ...
  const Url = createTokenClass("url", {
    ...
    hasProtocol() {
      ...
      return (
        tokens.length >= 2 && tokens[0].t !== LOCALHOST && tokens[1].t === COLON
      );
    },
  });
  ...
  function registerTokenPlugin(name, plugin) {
    if (typeof plugin !== "function") {
      throw new Error(
        `linkifyjs: Invalid token plugin ${plugin} (expects function)`,
      );
    }
    for (let i = 0; i < INIT.tokenQueue.length; i++) {
      if (name === INIT.tokenQueue[i][0]) {
        warn(
          `linkifyjs: token plugin "${name}" already registered - will be overwritten`,
        );
    ...
    if (INIT.initialized) {
      warn(
        `linkifyjs: already initialized - will not register token plugin "${name}" ${warnAdvice}`,
      );
    }
  }
  ...
  function registerPlugin(name, plugin) {
    ...
    for (let i = 0; i < INIT.pluginQueue.length; i++) {
      if (name === INIT.pluginQueue[i][0]) {
        warn(
          `linkifyjs: plugin "${name}" already registered - will be overwritten`,
        );
    ...
    if (INIT.initialized) {
      warn(
        `linkifyjs: already initialized - will not register plugin "${name}" ${warnAdvice}`,
      );
    }
  }
  ...
  function registerCustomProtocol(scheme, optionalSlashSlash) {
    ...
    if (INIT.initialized) {
      warn(
        `linkifyjs: already initialized - will not register custom scheme "${scheme}" ${warnAdvice}`,
      );
    }
...
3. "-" cannot repeat`);
  ...
  function test(str, type) {
    ...
    const tokens = tokenize(str);
    return (
      tokens.length === 1 && tokens[0].isLink && (!type || tokens[0].t === type)
    );
  }
