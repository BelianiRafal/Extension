...
window.ChecklistHighlighter = {
  ...
  getChecklistDataFromAPI: async function (issueId) {
    ...
    const translationsChecklist = data.checklists.find(
      (cl) =>
    ...
    if (!translationsChecklist) {
      console.warn(
        '[CHECKLIST API] Nie znaleziono checklisty "Newsletter Translations"'
      );
      return null;
    }
  ...
  highlightRows: async function () {
    ...
    console.log(
      '🎨 [CHECKLIST HIGHLIGHTER] Dane z API:',
    ...
    for (const div of allDivs) {
      ...
      if (text.includes('Newsletter Translations')) {
        newsletterChecklistContainer = div.closest(
          '.panel, [class*="panel"], [role="tabpanel"]'
        );
        if (newsletterChecklistContainer) {
          console.log(
            '✅ [CHECKLIST HIGHLIGHTER] Znaleziono kontener Newsletter Translations'
          );
          break;
        }
      }
    ...
    if (!newsletterChecklistContainer) {
      console.warn(
        '⚠️ [CHECKLIST HIGHLIGHTER] Nie znaleziono kontenera Newsletter Translations'
      );
      return 0;
    }
    ...
    const checklistRows = Array.from(allListItems).filter(
      (li) => li.querySelector('a[href*="change_log.php"]') !== null
    );

    console.log(
      `✅ [CHECKLIST HIGHLIGHTER] Znaleziono ${checklistRows.length} wierszy`
    );
    ...
    checklistRows.forEach((row, index) => {
      ...
      if (!changedBy) {
        console.log(
          `⏭️ [ROW ${index + 1}] Brak pola changed_by dla tableid=${tableid}`
        );
        return;
      }

      // Parse date from changed_by string (format: "Name 2025-11-25 14:30:00")
      const dateMatch = String(changedBy).match(
        /(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2}:\d{2})/
      );
      if (!dateMatch) {
        console.log(
          `⏭️ [ROW ${index + 1}] Nieprawidłowy format changed_by: ${changedBy}`
        );
        return;
      }
      ...
      if (color) {
        ...
        console.log(
          `✅ [ROW ${
            index + 1
          }] Podświetlono: tableid=${tableid}, data=${itemDateStr}, diffDays=${diffDays}`
        );
      }
    });

    console.log(
      `📊 [CHECKLIST HIGHLIGHTER] Zakończono: ${todayCount} wierszy z dzisiaj`
    );
    return todayCount;
  },
  ...
  safeHighlightRows: async function () {
    ...
    try {
      ...
      console.log(
        `📊 [CHECKLIST HIGHLIGHTER] Zwrócono todayCount: ${todayCount}`
      );
  ...
  init: function () {
    ...
    console.log(
      '✅ [CHECKLIST HIGHLIGHTER] Gotowe! Użyj: window.highlightChecklistByDate()'
    );
  },
};
