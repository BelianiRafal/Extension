...
window.FloatingChecklistUtils = {
  ...
  extractSpreadsheetInfo: function (url) {
    const result = {
      spreadsheetId: null,
      gid: null,
    };
    ...
    // because the link usually has gid in two versions: as a query parameter and as a hash, extract both
    const queryGidMatch = url.match(/[?&]gid=([^&#]+)/);
    const hashGidMatch = url.match(/#gid=([^&]+)/);

    result.gid = queryGidMatch
      ? queryGidMatch[1]
      : hashGidMatch
      ? hashGidMatch[1]
      : null;
