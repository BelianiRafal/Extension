  ...
  try {
    ...
    if (formData) {
      ...
      const clickCopyNameButton = () => {
        try {
          const copyNameButton =
            iframe.contentDocument.getElementById("copyName");

          if (copyNameButton) {
            console.log(
              "Znaleziono przycisk copyName, symulacja kliknięcia...",
            );
      ...
      while (clickAttempts < 3 && !clickSuccess) {
        clickAttempts++;
        console.log(
          `Próba kliknięcia przycisku copyName (${clickAttempts}/3)...`,
        );
        ...
        if (!clickSuccess) {
          console.log(
            "Próba kliknięcia nieudana, czekam i próbuję ponownie...",
          );
          await new Promise((resolve) => setTimeout(resolve, 1000));
        }
      }
      ...
      const clickUpdateButton = () => {
        try {
          ...
          const updateButtons = iframe.contentDocument.querySelectorAll(
            'input[name="update"], input[value="Update"]',
          );
      ...
      if (clickSuccess && updateSuccess) {
        ...
        new Notification(
          "Copy Name to Title completed, but Update may have failed.",
        );
      } else {
        new Notification(
          "Landing page date and name updated (without title copy).",
        );
      }
    }
  } catch (error) {
