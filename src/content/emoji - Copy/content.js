...
(function() {
    ...
    function fixContainerLayout() {
      ...
      const targetElements = document.getElementsByClassName('issue-module__container__2JAbV');
      
      if (targetElements.length > 0) {
        console.log('Znaleziono elementy do modyfikacji układu:', targetElements.length);
        ...
        const alternativeTargets = document.querySelectorAll('[class*="issue-module__container"]');
        if (alternativeTargets.length > 0) {
          console.log('Znaleziono alternatywne elementy do modyfikacji układu:', alternativeTargets.length);
          for (let i = 0; i < alternativeTargets.length; i++) {
            alternativeTargets[i].style.setProperty('display', 'block', 'important');
            console.log('Zmieniono styl dla alternatywnego elementu layout:', alternativeTargets[i]);
          }
        }
      }
    }
    ...
    function checkAndAddButton() {
      ...
      if (!window.location.href.includes('prologistics.info/react/logs/issue_logs/')) {
        return;
      }
      ...
      const buttonContainer = document.querySelector('.new-comment-module__buttons__3jd4H');
      if (!buttonContainer) {
        return;
      }
      
      // Sprawdźmy przyciski w nowym kontenerze
      const addCommentBtn = buttonContainer.querySelector('button[data-full-width="true"]:first-child');
      const corrActionBtn = buttonContainer.querySelector('button[data-full-width="true"]:last-child');
    ...
    function createEnhancedEmojiPicker() {
      ...
      const categories = [
        ...
        },
        {
          ...
          class: "emoji-head",
          emojis: [
            { visual: 'Head Trans', html: '<h4 style="color:#fff; background-color:#000; text-align:center; padding:20px;">PLS MAKE A TRANSLATIONS &#128512;<h4>' },
            { visual: 'Head Test', html: '<h4 style="color:#fff; background-color:#000; text-align:center; padding:20px;">PLS TEST NSLT & LP &#128512;<h4>' },
      ...
      categories.forEach((category, index) => {
        ...
        tab.addEventListener('click', () => {
          ...
          document.querySelector(`.emoji-category[data-index="${index}"]`).style.display = 'block';
        });
      ...
      searchInput.addEventListener('input', function() {
        ...
        if (searchTerm === '') {
          document.querySelectorAll('.emoji-category').forEach((category, index) => {
    ...
    function insertEmoji(htmlEntity) {
      ...
      textarea.value = text.substring(0, startPos) + htmlEntity + text.substring(endPos);
